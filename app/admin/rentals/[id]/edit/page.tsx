...
export default function EditRentalPage() {
  ...
  const fetchRental = async () => {
    try {
      ...
      if (!res.ok) {
        alert('Kiralama sözleşmesi bulunamadı')
      ...
      // Mevcut yenileme tarihine göre periyot tipini belirle
      let renewalPeriodType = 'manual'
      if (data.renewalDate) {
        const renewalDate = new Date(data.renewalDate)
        const startDate = new Date(data.startDate)
        
        // Her ayın biri kontrolü
        if (renewalDate.getDate() === 1) {
          renewalPeriodType = 'first-of-month'
        }
        // Her 3 ayda bir kontrolü (yaklaşık)
        else if (Math.abs((renewalDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)) > 85) {
          renewalPeriodType = 'quarterly'
        }
        // Her ayın aynı günü kontrolü
        else if (renewalDate.getDate() === startDate.getDate()) {
          renewalPeriodType = 'same-day'
        }
      }
      ...
      setFormData({
        ...
        vadeDays: data.vadeDays || 0,
        notes: data.notes || '',
      })

      // Seçili araçları ve fiyatlarını yükle
      const vehicleIds: string[] = []
      const prices: Record<string, number> = {}
      
      if (data.vehicles && Array.isArray(data.vehicles)) {
        data.vehicles.forEach((rv: any) => {
          if (rv.vehicleId) {
            vehicleIds.push(rv.vehicleId)
            prices[rv.vehicleId] = rv.price || 0
          } else if (rv.vehicle?.id) {
            vehicleIds.push(rv.vehicle.id)
            prices[rv.vehicle.id] = rv.price || 0
          }
        })
      }
      ...
      alert('Kiralama sözleşmesi yüklenirken hata oluştu')
      router.push('/admin/rentals')
    } finally {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-bold text-gray-800">Kiralama Sözleşmesi Düzenle</h1>
      </div>
